...
/* Drops the first value of a list of lists */
drop_first([_|T], T).
 ...
 * Written to look like sum/3 from clpfd, rel is the clpfd constraint to use (i.e. #=, #<, etc) */
prod([Prod_expr], Rel, Prod) :- call(Rel, Prod_expr, Prod). % executes the accumulated expression
prod([H1, H2 | T], Rel, Prod) :-
    prod([H1 * H2|T], Rel, Prod) % Builds up an expression for the product
.
